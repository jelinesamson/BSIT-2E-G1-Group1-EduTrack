    ...
    if ($_GET['api'] === 'checkout' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($cart as $item) {
            ...
            mysqli_query($conn, "UPDATE products SET quantity = $new_stock WHERE id = $pid");
             ...
             $incoming_res = mysqli_query($conn, "SELECT incoming_qty FROM products WHERE id = $pid");
    $row = mysqli_fetch_assoc($incoming_res);
    $incoming_qty = intval($row['incoming_qty']);

            $notes = "Sale $txnId";
            $admin = "Cashier";
            
            logJournal($conn, $pcode, $incoming_qty, $qty_sold, $notes, $admin);
        }
